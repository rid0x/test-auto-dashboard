...
const BASE = bladevilleConfig.baseUrl;

test.describe('Bladeville - Security Tests @security', () => {
  ...
  test.describe('SQL Injection', () => {
    ...
    test('login form rejects SQL injection in email', async ({ page }) => {
      ...
      const cookie = page.locator('.amgdprcookie-button.-allow').first();
      ...
      await page.locator('#password, #pass, input[name="login[password]"]').first().fill("' OR '1'='1");
      await page.locator('button.action.login, #send2').first().click({ force: true });
  ...
  test.describe('Cookie Security', () => {
    ...
    test('session cookies have HttpOnly and Secure flags', async ({ page, context }) => {
      ...
      const siteCookies = cookies.filter(c => c.domain.includes('bladeville'));
