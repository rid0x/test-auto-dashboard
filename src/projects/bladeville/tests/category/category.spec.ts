...
test.describe('Bladeville - Category @category @e2e', () => {
  ...
  test('should display product names and prices', async ({ page }) => {
    ...
    const productName = firstProduct.locator('.product-item-name, .product-item-link, a strong').first();
  ...
  test('should have product count in toolbar', async ({ page }) => {
    // Bladeville shows "Produkty 1-60 z 684" text
    const toolbar = page.locator('.toolbar-amount, :has-text("Produkty 1-")');
