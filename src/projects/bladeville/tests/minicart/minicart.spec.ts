...
test.describe('Bladeville - Minicart @minicart @e2e', () => {

  // @desc: Ikona minicart jest widoczna w headerze
  test('should display minicart icon in header', async ({ homePage, page }) => {
    await homePage.goto();
    const minicart = page.locator('[data-block="minicart"], a[href*="checkout/cart"], .minicart-wrapper');
  ...
  // @desc: Klikniecie ikony koszyka rozwija minicart lub przenosi do koszyka
  test('should open minicart or navigate to cart on click', async ({ productPage, page }) => {
    ...
    const toggle = page.locator('a[href*="checkout/cart"], .minicart-wrapper, [data-block="minicart"]').first();
    if (await toggle.isVisible().catch(() => false)) {
      await toggle.click({ force: true });
      await page.waitForTimeout(2000);

      // Either minicart dropdown appears OR we navigated to cart page
      const dropdown = page.locator('.block-minicart, #minicart-content-wrapper, .minicart-items, .aside-main');
