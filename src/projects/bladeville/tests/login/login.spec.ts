...
test.describe('Bladeville - Login @login @e2e', () => {
  ...
  test('should display login page correctly', async ({ loginPage, page }) => {
    ...
    await test.step('Verify login button', async () => {
      await expect(page.locator('button.action.login, button:has-text("Zaloguj się")').first()).toBeVisible();
    });
  ...
  test('should show error with empty password', async ({ loginPage, page }) => {
    await loginPage.login('test@bladeville.pl', '');
    expect(await loginPage.isOnLoginPage()).toBeTruthy();
  });
  ...
  // @desc: Link "Utwórz konto" jest widoczny i prowadzi do rejestracji
