...
test.describe('Bladeville - API Tests @api', () => {
  test.beforeAll(async () => {
    api = new MagentoApiClient(bladevilleConfig);
    await api.init();
  });
  ...
  test.describe('REST API @rest', () => {
    ...
    test('should search products via REST', async () => {
      const start = Date.now();
      const result = await api.searchProducts(bladevilleConfig.search.validQuery);
      await attachApiResponse(test.info(), `REST Search: "${bladevilleConfig.search.validQuery}"`, result, {
        method: 'GET', url: `${api.restBase}/products?search=${bladevilleConfig.search.validQuery}`, duration: Date.now() - start,
      });
      expect([200, 401, 404, 502]).toContain(result.status);
    ...
    test('should return empty for invalid search via REST', async () => {
      const start = Date.now();
      const result = await api.searchProducts(bladevilleConfig.search.invalidQuery);
      await attachApiResponse(test.info(), `REST Search (invalid): "${bladevilleConfig.search.invalidQuery}"`, result, {
    ...
    test('should authenticate customer via REST', async () => {
      try {
        ...
        const token = await api.getCustomerToken(
          bladevilleConfig.credentials.valid.email,
          bladevilleConfig.credentials.valid.password
        );
  ...
  test.describe('GraphQL API @graphql', () => {
    ...
    test('should search products via GraphQL', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(bladevilleConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Search: "${bladevilleConfig.search.validQuery}"`, result, {
    ...
    test('should return product details in GraphQL search', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(bladevilleConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Product Details: "${bladevilleConfig.search.validQuery}"`, result, {
        method: 'POST', url: api.graphqlUrl, duration: Date.now() - start,
      });
