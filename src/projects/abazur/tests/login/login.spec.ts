...
test.describe('Abazur - Login @login @e2e', () => {
  ...
  // @desc: Logowanie poprawnymi danymi konczy sie sukcesem (skip — reCAPTCHA v3)
  test('should login with valid credentials', async ({ loginPage, page, config }) => {
    test.skip(!config.credentials.valid.email, 'BRAK DANYCH: Ustaw ABAZUR_USER_EMAIL i ABAZUR_USER_PASSWORD w .env');
  ...
  // @desc: Bledne dane logowania wyswietlaja komunikat o bledzie (skip — reCAPTCHA v3)
  ...
  test('should show error with empty password', async ({ loginPage, page }) => {
    ...
    await test.step('Submit with empty password', async () => {
      await loginPage.login('test@abazur.pl', '');
    });
