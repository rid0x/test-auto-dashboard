...
test.describe('Szklaneczki - API Tests @api', () => {
  test.beforeAll(async () => {
    api = new MagentoApiClient(szklaneczkiConfig);
    await api.init();
  });
  ...
  test.describe('REST API @rest', () => {
    ...
    test('should search products via REST', async () => {
      const start = Date.now();
      const result = await api.searchProducts(szklaneczkiConfig.search.validQuery);
      await attachApiResponse(test.info(), `REST Search: "${szklaneczkiConfig.search.validQuery}"`, result, {
        method: 'GET', url: `${api.restBase}/products?search=${szklaneczkiConfig.search.validQuery}`, duration: Date.now() - start,
      });
      expect([200, 401, 404, 502]).toContain(result.status);
    ...
    test('should return empty for invalid search via REST', async () => {
      const start = Date.now();
      const result = await api.searchProducts(szklaneczkiConfig.search.invalidQuery);
      await attachApiResponse(test.info(), `REST Search (invalid): "${szklaneczkiConfig.search.invalidQuery}"`, result, {
    ...
    test('should authenticate customer via REST', async () => {
      try {
        ...
        const token = await api.getCustomerToken(
          szklaneczkiConfig.credentials.valid.email,
          szklaneczkiConfig.credentials.valid.password
        );
  ...
  test.describe('GraphQL API @graphql', () => {
    ...
    test('should search products via GraphQL', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(szklaneczkiConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Search: "${szklaneczkiConfig.search.validQuery}"`, result, {
    ...
    test('should return product details in GraphQL search', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(szklaneczkiConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Product Details: "${szklaneczkiConfig.search.validQuery}"`, result, {
        method: 'POST', url: api.graphqlUrl, duration: Date.now() - start,
      });
