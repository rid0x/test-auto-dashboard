...
const BASE = cornetteConfig.baseUrl;

test.describe('Cornette - Security Tests @security', () => {
  ...
  test.describe('SQL Injection', () => {
    ...
    test('login form rejects SQL injection in email', async ({ page }) => {
      ...
      const cookie = page.locator('.cky-btn-accept, [data-cky-tag="accept-button"]').first();
      if (await cookie.isVisible({ timeout: 3000 }).catch(() => false)) {
        await cookie.click();
        await page.waitForTimeout(500);
      }
      await page.evaluate(() => {
        document.querySelectorAll('.cky-overlay, .cky-consent-container').forEach(el => el.remove());
      }).catch(() => {});
      ...
      await page.locator('button.action.login, #send2, #bnt-social-login-authentication').first().click({ force: true });
  ...
  test.describe('Cookie Security', () => {
    ...
    test('session cookies have HttpOnly and Secure flags', async ({ page, context }) => {
      ...
      const siteCookies = cookies.filter(c => c.domain.includes('cornette'));
