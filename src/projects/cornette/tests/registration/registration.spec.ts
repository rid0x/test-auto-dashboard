...
test.describe('Cornette - Registration @registration @e2e', () => {
  ...
  test('should validate password mismatch', async ({ page }) => {
    ...
    // Cornette uses Polish validation message
  ...
  test('should register with valid data', async ({ registrationPage, page, config }) => {
    ...
    // Dismiss CookieYes overlay if present
    try {
      const acceptBtn = page.locator('.cky-btn-accept, [data-cky-tag="accept-button"]').first();
