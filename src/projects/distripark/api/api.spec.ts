...
test.describe('Distripark - API Tests @api', () => {
  ...
  test.describe('REST API @rest', () => {
    ...
    test('should search products via REST', async () => {
      ...
      await attachApiResponse(test.info(), `REST Search: "${distriparkConfig.search.validQuery}"`, result, {
        method: 'GET', url: `${api.restBase}/products?search=${distriparkConfig.search.validQuery}`, duration: Date.now() - start,
      });
      expect([200, 401, 404, 502]).toContain(result.status);
    ...
    test('should return empty for invalid search via REST', async () => {
      ...
      await attachApiResponse(test.info(), `REST Search (invalid): "${distriparkConfig.search.invalidQuery}"`, result, {
    ...
    test('should authenticate customer via REST', async () => {
      try {
        ...
        const token = await api.getCustomerToken(
          distriparkConfig.credentials.valid.email,
          distriparkConfig.credentials.valid.password
        );
  ...
  test.describe('GraphQL API @graphql', () => {
    ...
    test('should search products via GraphQL', async () => {
      ...
      await attachApiResponse(test.info(), `GraphQL Search: "${distriparkConfig.search.validQuery}"`, result, {
    ...
    test('should return product details in GraphQL search', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(distriparkConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Product Details: "${distriparkConfig.search.validQuery}"`, result, {
        method: 'POST', url: api.graphqlUrl, duration: Date.now() - start,
      });
