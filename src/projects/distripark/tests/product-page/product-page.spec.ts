...
test.describe('Distripark - Product Page @product-page @e2e', () => {
  ...
  test('should display breadcrumbs', async ({ page, config }) => {
    await page.goto(`${config.baseUrl}${config.category.url}`);
    await page.locator('.product-item').first().waitFor({ state: 'visible', timeout: 10000 });
    await page.locator('.product-item a').first().click();
    await page.waitForLoadState('load');
    await expect(page.locator('.breadcrumbs, .breadcrumb').first()).toBeVisible();
  });

  test('should have product reviews section', async ({ page }) => {
    const reviews = page.locator('#tab-label-reviews, .product-reviews-summary');
