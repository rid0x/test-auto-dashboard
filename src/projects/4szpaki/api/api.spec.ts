...
test.describe('4szpaki - API Tests @api', () => {
  test.beforeAll(async () => {
    api = new MagentoApiClient(szpakiConfig);
    await api.init();
  });
  ...
  test.describe('REST API @rest', () => {
    ...
    test('should search products via REST', async () => {
      ...
      await attachApiResponse(test.info(), `REST Search: "${szpakiConfig.search.validQuery}"`, result, {
        method: 'GET', url: `${api.restBase}/products?search=${szpakiConfig.search.validQuery}`, duration: Date.now() - start,
      });
      expect([200, 401, 404, 502]).toContain(result.status);
    ...
    test('should return empty for invalid search via REST', async () => {
      ...
      await attachApiResponse(test.info(), `REST Search (invalid): "${szpakiConfig.search.invalidQuery}"`, result, {
    ...
    test('should authenticate customer via REST', async () => {
      try {
        ...
        const token = await api.getCustomerToken(
          szpakiConfig.credentials.valid.email,
          szpakiConfig.credentials.valid.password
        );
  ...
  test.describe('GraphQL API @graphql', () => {
    ...
    test('should search products via GraphQL', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(szpakiConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Search: "${szpakiConfig.search.validQuery}"`, result, {
    ...
    test('should return product details in GraphQL search', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(szpakiConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Product Details: "${szpakiConfig.search.validQuery}"`, result, {
        method: 'POST', url: api.graphqlUrl, duration: Date.now() - start,
      });
