...
test.describe('4szpaki - Homepage @homepage @e2e', () => {
  test.beforeEach(async ({ homePage }) => {
    await homePage.goto();
  });

  test('should load homepage successfully', async ({ page }) => {
    await expect(page).toHaveURL(/4szpaki\.pl/);
  });
  ...
  test('should load without console errors', async ({ page }) => {
    ...
    await page.goto('https://4szpaki.pl', { waitUntil: 'load' });
  ...
  test('should be responsive - mobile viewport', async ({ page }) => {
    ...
    await page.goto('https://4szpaki.pl', { waitUntil: 'domcontentloaded' });
