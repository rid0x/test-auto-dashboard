...
test.describe('4szpaki - Search @search @e2e', () => {
  ...
  test('should show no results for invalid query', async ({ page, config }) => {
    await page.goto(`https://4szpaki.pl/catalogsearch/result/?q=${config.search.invalidQuery}`, { waitUntil: 'load' });
  ...
  test('should display product info in results', async ({ page, config }) => {
    await page.goto(`https://4szpaki.pl/catalogsearch/result/?q=${config.search.validQuery}`, { waitUntil: 'load' });
