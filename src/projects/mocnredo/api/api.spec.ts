...
import { mocnredoConfig } from '../../../../config/mocnredo.config';

let api: MagentoApiClient;

test.describe('Mocnredo - API Tests @api', () => {
  test.beforeAll(async () => {
    api = new MagentoApiClient(mocnredoConfig);
    await api.init();
  });
  ...
  test.describe('REST API @rest', () => {
    ...
    test('should search products via REST', async () => {
      const result = await api.searchProducts(mocnredoConfig.search.validQuery);
      expect(result.status).toBe(200);
      expect(result.body?.items?.length).toBeGreaterThan(0);
    });

    test('should return empty for invalid search via REST', async () => {
      const result = await api.searchProducts(mocnredoConfig.search.invalidQuery);
      expect(result.status).toBe(200);
    ...
    test('should authenticate customer via REST', async () => {
      try {
        const token = await api.getCustomerToken(
          mocnredoConfig.credentials.valid.email,
          mocnredoConfig.credentials.valid.password
        );
  ...
  test.describe('GraphQL API @graphql', () => {
    test('should search products via GraphQL', async () => {
      const result = await api.graphqlSearchProducts(mocnredoConfig.search.validQuery);
      expect(result.status).toBe(200);
