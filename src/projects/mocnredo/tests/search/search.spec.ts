...
test.describe('Mocnredo - Search @search @e2e', () => {
  ...
  test('should show no results for invalid query', async ({ page, config }) => {
    ...
    await page.goto(`https://mocnredo.pl/catalogsearch/result/?q=${config.search.invalidQuery}`, { waitUntil: 'load' });
    await page.waitForTimeout(2000);
  ...
  test('should display product info in results', async ({ page, config }) => {
    await page.goto(`https://mocnredo.pl/catalogsearch/result/?q=${config.search.validQuery}`, { waitUntil: 'load' });
    await page.waitForTimeout(2000);
