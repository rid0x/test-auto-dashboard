...
test.describe('Mocnredo - Homepage @homepage @e2e', () => {
  test.beforeEach(async ({ homePage }) => {
    await homePage.goto();
  });

  test('should load homepage successfully', async ({ page }) => {
    await expect(page).toHaveURL(/mocnredo\.pl/);
  });
  ...
  test('should load without console errors', async ({ page }) => {
    ...
    await page.goto('https://mocnredo.pl', { waitUntil: 'load' });
  ...
  test('should be responsive - mobile viewport', async ({ page }) => {
    ...
    await page.goto('https://mocnredo.pl', { waitUntil: 'domcontentloaded' });
