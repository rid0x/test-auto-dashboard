...
test.describe('Pierrerene - Homepage @homepage @e2e', () => {
  ...
  test('should load homepage successfully', async ({ page }) => {
    await expect(page).toHaveURL(/pierrerene\.pl/);
  });
  ...
  test('should load without console errors', async ({ page, config }) => {
    ...
    await page.goto(`${config.baseUrl}/pl/`, { waitUntil: 'load' });
  ...
  test('should be responsive - mobile viewport', async ({ page, config }) => {
    ...
    await page.goto(`${config.baseUrl}/pl/`, { waitUntil: 'domcontentloaded' });

    const hasSearch = await page.locator('#search, .js-search-input, input[name="q"]').first().isVisible().catch(() => false);
    const hasCart = await page.locator('.minicart-wrapper, a.showcart, [data-block="minicart"]').first().isVisible().catch(() => false);
