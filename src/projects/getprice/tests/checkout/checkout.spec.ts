...
test.describe('Getprice - Checkout @checkout @e2e', () => {
  ...
  test('should display all shipping address fields', async ({ cartPage, page }) => {
    ...
    await cartPage.proceedToCheckout();
    // Wait for KnockoutJS
    const checkoutEl = page.locator("#customer-email, input[name=\"username\"], .checkout-shipping-address, #checkout-step-shipping, .opc-wrapper");
    await expect(checkoutEl.first()).toBeVisible({ timeout: 30000 });
  ...
  test('should display shipping methods after filling address', async ({ cartPage, page, config }) => {
    await cartPage.goto();
    await cartPage.proceedToCheckout();

    // Getprice Hyva checkout — czekaj na formularz
    const checkoutForm = page.locator('#customer-email, input[name="username"], input[name="firstname"], input[name="email"], .checkout-shipping-address, #checkout-step-shipping, form[id*="checkout"]');
    if (!(await checkoutForm.first().isVisible({ timeout: 30000 }).catch(() => false))) {
      test.skip(true, 'Checkout form nie załadował się');
      return;
    }

    // Kliknij guest checkout jesli dostepny
    const guestBtn = page.locator('button:has-text("Kup jako gość"), button:has-text("Kontynuuj"), button:has-text("Zakupy bez logowania"), button:has-text("guest")');
    if (await guestBtn.first().isVisible({ timeout: 3000 }).catch(() => false)) {
      await guestBtn.first().click();
      await page.waitForTimeout(2000);
    }

    // Wypelnij formularz — uzywaj wielu selektorow (Hyva + Luma)
    ...
    await fillField('#customer-email, input[name="username"], input[name="email"]', 'test@test.pl');
    await fillField('input[name="firstname"]', 'Test');
    await fillField('input[name="lastname"]', 'User');
    await fillField('input[name="street[0]"], input[name="street"]', 'Testowa 1');
    await fillField('input[name="postcode"]', '00-001');
    await fillField('input[name="city"]', 'Warszawa');
    await fillField('input[name="telephone"]', '500100200');
    await page.waitForTimeout(2000);

    // Sprawdz metody dostawy
    const shippingMethods = page.locator('#checkout-shipping-method-load, .table-checkout-shipping-method, input[type="radio"][name*="shipping"], input[type="radio"][name*="ko_unique"], [x-show*="shipping"], .shipping-method');
