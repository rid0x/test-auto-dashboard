 ...
 */
test.describe('Getprice - Smoke Tests @smoke', () => {

  // === HOMEPAGE ===
  test('smoke: homepage loads', async ({ homePage, page }) => {
    await homePage.goto();
  ...
  // === CATEGORY → PRODUCT ===
  test('smoke: category shows products', async ({ categoryPage, page, config }) => {
    ...
    const screenshot = await page.screenshot();
    await test.info().attach('Category page', { body: screenshot, contentType: 'image/png' });
  });
  ...
  test('smoke: navigate from category to product', async ({ categoryPage, page, config }) => {
    ...
    const screenshot = await page.screenshot();
    await test.info().attach('Product from category', { body: screenshot, contentType: 'image/png' });
  });

  // === PRODUCT PAGE ===
  test('smoke: product page displays correctly', async ({ productPage, page }) => {
  ...
  test.skip('smoke: place order', async ({ productPage, checkoutPage, page }) => {
    // TODO: Odblokować gdy będzie gotowe
    // Ten test przechodzi cały flow do końca: wypełnia adres, wybiera dostawę,
    // wybiera płatność i klika "Kupuję i płacę" / "Złóż zamówienie"
    // Na razie zablokowany żeby nie składać prawdziwych zamówień
  });
});
