...
import { enteloConfig } from '../../../../config/entelo.config';

let api: MagentoApiClient;

test.describe('Entelo - API Tests @api', () => {
  test.beforeAll(async () => {
    api = new MagentoApiClient(enteloConfig);
    await api.init();
  });
  ...
  test.describe('REST API @rest', () => {
    ...
    test('should search products via REST', async () => {
      const start = Date.now();
      const result = await api.searchProducts(enteloConfig.search.validQuery);
      await attachApiResponse(test.info(), `REST Search: "${enteloConfig.search.validQuery}"`, result, {
        method: 'GET', url: `${api.restBase}/products?search=${enteloConfig.search.validQuery}`, duration: Date.now() - start,
      });
      expect([200, 401, 404, 502]).toContain(result.status);
    ...
    test('should return empty for invalid search via REST', async () => {
      const start = Date.now();
      const result = await api.searchProducts(enteloConfig.search.invalidQuery);
      await attachApiResponse(test.info(), `REST Search (invalid): "${enteloConfig.search.invalidQuery}"`, result, {
    ...
    test('should authenticate customer via REST', async () => {
      try {
        ...
        const token = await api.getCustomerToken(
          enteloConfig.credentials.valid.email,
          enteloConfig.credentials.valid.password
        );
  ...
  test.describe('GraphQL API @graphql', () => {
    ...
    test('should search products via GraphQL', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(enteloConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Search: "${enteloConfig.search.validQuery}"`, result, {
    ...
    test('should return product details in GraphQL search', async () => {
      const start = Date.now();
      const result = await api.graphqlSearchProducts(enteloConfig.search.validQuery);
      await attachApiResponse(test.info(), `GraphQL Product Details: "${enteloConfig.search.validQuery}"`, result, {
        method: 'POST', url: api.graphqlUrl, duration: Date.now() - start,
      });
