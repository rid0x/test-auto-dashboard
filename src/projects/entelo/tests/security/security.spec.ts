...
const BASE = enteloConfig.baseUrl;

test.describe('Entelo - Security Tests @security', () => {
  ...
  test.describe('SQL Injection', () => {
    ...
    test('search input rejects SQL injection', async ({ page }) => {
      ...
      const status = page.url().includes('catalogsearch') || page.url().includes('entelo');
    ...
    test('login form rejects SQL injection in email', async ({ page }) => {
      ...
      const cookie = page.locator('#CookiebotDialogBodyLevelButtonLevelOptinAllowAll').first();
      ...
      await page.locator('form[action*="loginPost"] #email, #email:visible').first().fill("admin' OR '1'='1");
      await page.locator('#password:visible, form[action*="loginPost"] input[type="password"]').first().fill("' OR '1'='1");
      await page.locator('form[action*="loginPost"] button[type="submit"], button.action.login.primary').first().click({ force: true });
  ...
  test.describe('Cookie Security', () => {
    ...
    test('session cookies have HttpOnly and Secure flags', async ({ page, context }) => {
      ...
      const siteCookies = cookies.filter(c => c.domain.includes('entelo'));
