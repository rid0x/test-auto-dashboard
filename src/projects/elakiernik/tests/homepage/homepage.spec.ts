...
test.describe('Elakiernik - Homepage @homepage @e2e', () => {
  test.beforeEach(async ({ homePage }) => {
    await homePage.goto();
  });

  test('should load homepage successfully', async ({ page }) => {
    await expect(page).toHaveURL(/e-lakiernik\.net/);
  });
  ...
  test('should load without console errors', async ({ page, config }) => {
    ...
    const criticalErrors = errors.filter(e =>
      !e.includes('favicon') && !e.includes('analytics') && !e.includes('gtm') &&
      !e.includes('GTM') && !e.includes('google') && !e.includes('cloudflare') &&
      !e.includes('cookie') && !e.includes('cdn') && !e.includes('third-party') &&
