...
test.describe('Elakiernik - Security Tests @security', () => {
  ...
  test.describe('SQL Injection', () => {
    ...
    test('login form rejects SQL injection in email', async ({ page }) => {
      ...
      const cookie = page.locator('#CybotCookiebotDialogBodyLevelButtonLevelOptinAllowAll').first();
  ...
  test.describe('Cookie Security', () => {
    ...
    test('session cookies have HttpOnly and Secure flags', async ({ page, context }) => {
      ...
      const siteCookies = cookies.filter(c => c.domain.includes('elakiernik'));
