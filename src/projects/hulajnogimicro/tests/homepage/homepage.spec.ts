...
test.describe('Hulajnogimicro - Homepage @homepage @e2e', () => {
  test.beforeEach(async ({ homePage }) => {
    await homePage.goto();
  });

  // @desc: Strona glowna laduje sie poprawnie pod adresem hulajnogimicro.pl
  test('should load homepage successfully', async ({ page }) => {
    await expect(page).toHaveURL(/hulajnogimicro\.pl/);
  ...
  // @desc: Pole wyszukiwania jest obecne na stronie glownej (ukryte w headerze)
  test('should have search input in header', async ({ page }) => {
    // Hulajnogimicro has a hidden search input (#search) that opens via search icon
    const searchInput = page.locator('#search, input.cs-header-search__input');
    await expect(searchInput.first()).toBeAttached();

    const screenshot = await page.screenshot();
    await test.info().attach('Search input', { body: screenshot, contentType: 'image/png' });
  });

  // @desc: Menu nawigacji jest widoczne na stronie glownej
  test('should display navigation menu', async ({ page }) => {
    const nav = page.locator('.nav-sections, nav.navigation, nav[aria-label="Main Navigation"]').first();
  ...
  test('should display cart icon', async ({ page }) => {
    const cart = page.locator('a[href*="checkout/cart"]');
  ...
  test('should load without console errors', async ({ page, config }) => {
    ...
    const criticalErrors = errors.filter(
      e => !e.includes('favicon') && !e.includes('analytics') && !e.includes('gtm')
        ...
        && !e.includes('cookie') && !e.includes('opineo') && !e.includes('Cookiebot')
        && !e.includes('trustedshops') && !e.includes('CORS') && !e.includes('net::ERR')
        && !e.includes('ocdn') && !e.includes('youtube')
    );
  ...
  test('should be responsive - mobile viewport', async ({ page, config }) => {
    ...
    const mobileNav = page.locator(
      '.nav-toggle, .action.nav-toggle, button[aria-label="Menu"], .hamburger-menu, button:has-text("Menu")'
    );
