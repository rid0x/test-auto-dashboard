...
test.describe('Hulajnogimicro - Cart @cart @e2e', () => {
  ...
  test('should show mini cart after adding product', async ({ productPage, page }) => {
    ...
    // Hulajnogimicro minicart shows in offcanvas dialog
    const minicart = page.locator('.minicart-wrapper, a[href*="checkout/cart"]');
    const count = await minicart.count();
